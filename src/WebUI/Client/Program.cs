...
using RapidBlazor21.WebUI.Client;
using RapidBlazor21.WebUI.Client.Authorization;
...
builder.Services.AddHttpClient("RapidBlazor21.WebUI.ServerAPI", client => client.BaseAddress = new Uri(builder.HostEnvironment.BaseAddress))
    .AddHttpMessageHandler<BaseAddressAuthorizationMessageHandler>();

// Supply HttpClient instances that include access tokens when making requests to the server project
builder.Services.AddScoped(sp => sp.GetRequiredService<IHttpClientFactory>().CreateClient("RapidBlazor21.WebUI.ServerAPI"));
